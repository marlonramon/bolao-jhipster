...
export class CampeonatoComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.campeonatoService.query({
            ...
            (res: HttpResponse<Campeonato[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: Campeonato) {
        return item.id;
    }
    registerChangeInCampeonatoes() {
        this.eventSubscriber = this.eventManager.subscribe('campeonatoListModification', (response) => this.reset());
    }
