...
public interface BolaoRepository extends JpaRepository<Bolao, Long> {
    ...
    @Query(" select new br.com.bolao.service.dto.RankingDTO(bolao.id, "
			...
			+ "        sum(CASE "
			+ "           when (apostas.pontuacao >= bolao.pontosAcertoResultado and apostas.pontuacao < bolao.pontosAcertoDoisPlacares) then 1 "
			+ "           else 0"
			+ "        end) as aposta_resultado,"
			+ "        sum(CASE "
			+ "           when (apostas.pontuacao >= bolao.pontosAcertoUmPlacar and apostas.pontuacao < bolao.pontosAcertoResultado) then 1 "
			+ "           else 0"
